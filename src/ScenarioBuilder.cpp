...
bool ScenarioBuilder::getRecursiveFault() {
...
void ScenarioBuilder::setRecursiveFault(bool value) {
	pthread_mutex_lock(&hMutex);
	recursiveFault = value;
	pthread_mutex_unlock(&hMutex);
}

void ScenarioBuilder::addMechanism(const std::string& name, std::unique_ptr<Mechanism> mech) {
	mechanisms[name] = std::move(mech);
}

void ScenarioBuilder::loadCommandsFromFile(const std::string& filename) {
	commandQueue.clear();
...
void ScenarioBuilder::saveCommandsToFile(const std::string& filename) {
...
void ScenarioBuilder::stopAllMechanisms() {
	for (const auto& pair : mechanisms) {
		pair.second->stop();
	}
}

void ScenarioBuilder::executeCommands() {
	for (const auto& cmd : commandQueue) {
		executeSingleCommand(cmd);
	}
}

void ScenarioBuilder::executeSingleCommand(const std::string& command) {
	recursiveCounter += 1;
...
void ScenarioBuilder::MoveToPosition(const std::string& mechName, int targetPosition) {
	auto it = mechanisms.find(mechName);
...
void ScenarioBuilder::waitTime(int ms) {
	std::this_thread::sleep_for(std::chrono::milliseconds(ms));
}

void ScenarioBuilder::waitUntilPosition(const std::string& mechName, uint32_t position) {
	auto it = mechanisms.find(mechName);
	if (it == mechanisms.end()) return;
	Mechanism* mech = it->second.get();
...
void ScenarioBuilder::waitUntilMoreThanPosition(const std::string& mechName, uint32_t position) {
	auto it = mechanisms.find(mechName);
	if (it == mechanisms.end()) return;
	Mechanism* mech = it->second.get();
	while (mech->GetPosition(true) < position) {
...
void ScenarioBuilder::waitUntilLessThanPosition(const std::string& mechName, uint32_t position) {
	auto it = mechanisms.find(mechName);
	if (it == mechanisms.end()) return;
	Mechanism* mech = it->second.get();
	while (mech->GetPosition(true) > position) {
...
void ScenarioBuilder::saveMechanismsToFile(const std::string& filename) {
	std::ofstream file(filename);
	for (const auto& pair : mechanisms) {
...
bool ScenarioBuilder::detectFault() {
	for (const auto& pair : mechanisms) {
...
void ScenarioBuilder::loadMechanismsFromFile(const std::string& filename) {
	mechanisms.clear();
	std::ifstream file(filename);
	std::string line;
...
std::vector<MechanismState> ScenarioBuilder::getMechanismsStateList() {
